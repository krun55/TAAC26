...
USER_ACTIVITY_SEQ_DOMAINS = ('seq_a', 'seq_b', 'seq_c', 'seq_d')
USER_ACTIVITY_WINDOWS = (86400, 3 * 86400, 5 * 86400)


def timestamp_to_hour_dow(
    timestamp: "npt.NDArray[np.int64]",
    offset_hours: int,
) -> Tuple["npt.NDArray[np.int64]", "npt.NDArray[np.int64]"]:
    """Convert Unix-second timestamps to hour-of-day and Monday=0 DOW ids."""
    ts = np.asarray(timestamp, dtype=np.int64) + int(offset_hours) * 3600
    hour = (ts // 3600) % 24
    dow = ((ts // 86400) + 3) % 7
    return hour.astype(np.int64, copy=False), dow.astype(np.int64, copy=False)


def _validate_abs_time_array(
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        use_user_activity_dense: bool = False,
    ) -> None:
        """
        Args:
            ...
            time_zone_offset_hours: offset applied before deriving hour/DOW.
            user_abs_time_missing_as_padding: if True, user-side absolute
                time ids reserve 0 for missing timestamps and valid ids are
                shifted to hour 1..24 / dayofweek 1..7.
            use_user_activity_dense: if True, append synthetic user activity
                dense features derived from the truncated sequence histories.
        """
        super().__init__()
        ...
        self.time_zone_offset_hours = int(time_zone_offset_hours)
        self.user_abs_time_missing_as_padding = bool(user_abs_time_missing_as_padding)
        self.use_user_activity_dense = bool(use_user_activity_dense)
        ...
        for fid, dim in self._item_dense_cols:
            ...
            if ci is None:
                raise KeyError(
                    f"schema item_dense fid={fid} requires column "
                    f"item_dense_feats_{fid}")
        ...
        logging.info(
            ...
            f"user_abs_time_missing_as_padding={self.user_abs_time_missing_as_padding}, "
            f"use_user_activity_dense={self.use_user_activity_dense}")
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        self._user_activity_len_offsets: Dict[str, int] = {}
        self._user_activity_count_offsets: Dict[Tuple[str, int], int] = {}
        if self.use_user_activity_dense:
            existing_dense_fids = set(self.user_dense_schema.feature_ids)
            conflicts = sorted(existing_dense_fids & set(USER_ACTIVITY_DENSE_FIDS))
            if conflicts:
                raise ValueError(
                    f"activity dense fid conflict in user_dense schema: {conflicts}")
            if len(self._user_dense_cols) != len(existing_dense_fids):
                raise ValueError("duplicate fid detected in user_dense schema")

            for fid, domain in zip(USER_ACTIVITY_DENSE_FIDS[:4],
                                   USER_ACTIVITY_SEQ_DOMAINS):
                self.user_dense_schema.add(fid, 1)
                self._user_activity_len_offsets[domain] = (
                    self.user_dense_schema.get_offset_length(fid)[0])
            count_fids = USER_ACTIVITY_DENSE_FIDS[4:]
            for idx, fid in enumerate(count_fids):
                domain = USER_ACTIVITY_SEQ_DOMAINS[idx // len(USER_ACTIVITY_WINDOWS)]
                window_idx = idx % len(USER_ACTIVITY_WINDOWS)
                self.user_dense_schema.add(fid, 1)
                self._user_activity_count_offsets[(domain, window_idx)] = (
                    self.user_dense_schema.get_offset_length(fid)[0])

        # ---- item_dense: optional [[fid, dim], ...] ----
        self.item_dense_schema: FeatureSchema = FeatureSchema()
        self._item_dense_cols: List[List[int]] = raw.get('item_dense') or []
        for fid, dim in self._item_dense_cols:
        ...
        if self.use_user_activity_dense:
            missing_activity_domains = [
                d for d in USER_ACTIVITY_SEQ_DOMAINS if d not in self.seq_domains
            ]
            if missing_activity_domains:
                raise ValueError(
                    "use_user_activity_dense=True requires sequence domains "
                    f"{missing_activity_domains}")
    ...
    def _write_user_activity_dense(
        self,
        ...
        """Write log activity features for one sequence domain into user_dense."""
        if not self.use_user_activity_dense:
            return
        len_offset = self._user_activity_len_offsets.get(domain)
        if len_offset is None:
            return

        user_dense[:, len_offset] = np.log1p(
            lengths.astype(np.float32, copy=False))
        if ts_padded is None:
            return

        B, max_len = ts_padded.shape
        pos = np.arange(max_len, dtype=np.int64).reshape(1, -1)
        anchor = timestamps.reshape(B, 1)
        age = anchor - ts_padded
        hist_mask = (
            (pos < lengths.reshape(B, 1))
            & (anchor > 0)
            & (ts_padded > 0)
            & (age > 0)
        )
        for window_idx, window_seconds in enumerate(USER_ACTIVITY_WINDOWS):
            offset = self._user_activity_count_offsets[(domain, window_idx)]
            counts = (hist_mask & (age <= window_seconds)).sum(axis=1)
            user_dense[:, offset] = np.log1p(
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        user_hour, user_dow = timestamp_to_hour_dow(
            timestamps, self.time_zone_offset_hours)
        if self.user_abs_time_missing_as_padding:
            valid_user_ts = timestamps > 0
            shifted_hour = np.zeros_like(user_hour)
            shifted_dow = np.zeros_like(user_dow)
            shifted_hour[valid_user_ts] = user_hour[valid_user_ts] + 1
            shifted_dow[valid_user_ts] = user_dow[valid_user_ts] + 1
            user_hour = _validate_abs_time_array(shifted_hour, 0, 24, 'hour')
            user_dow = _validate_abs_time_array(shifted_dow, 0, 7, 'dayofweek')
        else:
            user_hour = _validate_abs_time_array(user_hour, 0, 23, 'hour')
            user_dow = _validate_abs_time_array(user_dow, 0, 6, 'dayofweek')
        if self.is_training:
        ...
        result = {
            ...
            'hour': torch.from_numpy(user_hour),
            'dayofweek': torch.from_numpy(user_dow),
            'user_id': user_ids,
        ...
        for domain in self.seq_domains:
            ...
            seq_hour[:] = 0
            seq_dow = self._buf_seq_dow[domain][:B]
            seq_dow[:] = 0
            ts_padded = None
            if ts_ci is not None:
                ...
                seq_hour_raw, seq_dow_raw = timestamp_to_hour_dow(
                    ts_padded, self.time_zone_offset_hours)
                valid_ts = ts_padded > 0
                seq_hour[valid_ts] = seq_hour_raw[valid_ts] + 1
                seq_dow[valid_ts] = seq_dow_raw[valid_ts] + 1

            self._write_user_activity_dense(
                user_dense, domain, lengths, ts_padded, timestamps)
            result[f'{domain}_time_bucket'] = torch.from_numpy(time_bucket.copy())
            result[f'{domain}_hour'] = torch.from_numpy(seq_hour.copy())
            result[f'{domain}_dayofweek'] = torch.from_numpy(seq_dow.copy())
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    train_dataset = PCVRParquetDataset(
        ...
        user_abs_time_missing_as_padding=user_abs_time_missing_as_padding,
